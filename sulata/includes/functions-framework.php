...
//FTP transfer
if (!function_exists('suFtpTransfer')) {

    function suFtpTransfer($hostName, $loginId, $password, $sourceFile, $destinationFile, $debug = FALSE) {
...
if (!function_exists('suInvoiceToPDF')) {

    //$headerArray=array('Col 1','Col 2','Col 3');
    function suInvoiceToPDF($sql, $outputFileName) {
  ...
  global $getSettings;
       ...
       $row = suFetch($result);
          $outputFileName = $row['order__Number'].'.pdf';
            $print_invoice = '<div style="width:300px;"><h1 style="text-align:center;">'.$getSettings['site_tagline'].'</h1><br><br>';
           
              $print_invoice .= '<p><b>Order No.</b>: ' . $row['order__Number'] . '<br>Customer: ' . $row['order__Customer_Name'] . '<br>';
            if($row['order__Mobile_Number']!=""){
                $print_invoice.='Mobile: '.$row['order__Mobile_Number'].'<br>';
            }
$print_invoice .='Date: ' . $row['orderDate'] . '<br><br>';
 $sql_invoice_details = "SELECT SUM(orderdet__Quantity) AS orderdet__Quantity,orderdet__Name,orderdet__Code,orderdet__Price FROM sulata_order_details  WHERE orderdet__Order = '" . $row['order_id'] . "' GROUP BY orderdet__Product";
    $rs_invoice_details = suQuery($sql_invoice_details);
    while ($row_invoice_deatils = suFetch($rs_invoice_details)) {
        $print_invoice.=''. $cnt = ($cnt + 1) . ') ' . $row_invoice_deatils['orderdet__Quantity'] . ' x ' . $row_invoice_deatils['orderdet__Name'] . ', @' . number_format($row_invoice_deatils['orderdet__Price'], 2) . ' = ' . number_format($row_invoice_deatils['orderdet__Quantity'] * $row_invoice_deatils['orderdet__Price'], 2) . '<br><br>';
    }
 $print_invoice.='<br>
...
';
    ...
    if ($row['order__Notes'] != "") {
        $print_invoice.='
Instructions: <br>
' . nl2br($row['order__Notes']) . '<br>
';
    }
    $print_invoice .='Cash Received: '.  number_format($row['order__Cash_Recieved'],2).'<br>Balance: '.number_format($balance,2).'
</p>
...
';
        ...
        // Print text using writeHTMLCell()
        $pdf->writeHTMLCell(0, 0, '', '', $print_invoice, 0, 1, 0, true, '', true);
        ...
        // END OF FILE
        //====================
        //suInvoiceKitchenToPDF($sql, $outputFileName);
    }

}
if (!function_exists('suKitchenInvoiceToPDF')) {

    //$headerArray=array('Col 1','Col 2','Col 3');
    function suKitchenInvoiceToPDF($sql, $outputFileName) {
  ...
  global $getSettings;
       ...
       $row = suFetch($result);
          $outputFileName = $row['order__Number'].'-kitchen.pdf';
            $print_invoice_kitchen = '<div style="width:300px;"><h1 style="text-align:center;">'.$getSettings['site_tagline'].'</h1><br><br>';
           
              $print_invoice_kitchen .= '<p><b>Order No.</b>: ' . $row['order__Number'] . '<br>Customer: ' . $row['order__Customer_Name'] . '<br>';
            if($row['order__Mobile_Number']!=""){
                $print_invoice_kitchen.='Mobile: '.$row['order__Mobile_Number'].'<br>';
            }
$print_invoice_kitchen .='Date: ' . $row['orderDate'] . '<br><br>';
 $sql_invoice_details = "SELECT SUM(orderdet__Quantity) AS orderdet__Quantity,orderdet__Name,orderdet__Code,orderdet__Price FROM sulata_order_details  WHERE orderdet__Order = '" . $row['order_id'] . "' GROUP BY orderdet__Product";
    $rs_invoice_details = suQuery($sql_invoice_details);
    while ($row_invoice_deatils = suFetch($rs_invoice_details)) {
...
';
        ...
        // Print text using writeHTMLCell()
        $pdf->writeHTMLCell(0, 0, '', '', $print_invoice_kitchen, 0, 1, 0, true, '', true);

...
checkLogin();
$pageName='Manage Uploads';$pageTitle='Manage Uploads';
...
$sqlSelect = "SELECT upload__ID,upload__Title,upload__Picture ";
$sqlFrom = " FROM sulata_uploads WHERE upload__dbState='Live'";
$sql = $sqlSelect . $sqlFrom;

//Download CSV
if (suSegment(1) == 'stream-csv' && $downloadAccessCSV == TRUE) {
    $outputFileName = 'uploads.csv';
                                ...
                                <form class="form-horizontal" name="searchForm" id="searchForm" method="get" action="">
                                    <fieldset id="search-area1">
                                        ...
                                        <div class="lineSpacer clear"></div>
                                         <div class="pull-right"><a style="text-decoration:underline !important;" href="<?php echo ADMIN_URL;?>uploads/">Clear search.</a></div>
                                        </div>
                                        <?php } ?>
                                    </fieldset>
                                </form>
...
<?php
if ($_GET['q'] != '') {
        $where .= " AND upload__Title LIKE '%" . suStrip($_GET['q']) . "%' ";
    }
...
?>
                                        <tr>
                                            ...
                                            <td><?php echo suUnstrip($row['upload__Title']);?></td>
<td><?php
...
<div>&nbsp;</div>
    ...
    <?php if ($editAccess == TRUE) { ?>
                                                <a href="<?php echo ADMIN_URL;?>uploads-update/<?php echo $row['upload__ID'];?>/"><img border="0" src="<?php echo BASE_URL; ?>sulata/images/edit.png" title="<?php echo EDIT_RECORD; ?>"/></a>
                                                    <?php } ?>
<?php if ($deleteAccess == TRUE) { ?>
                                                <a onclick="return delRecord(this, '<?php echo CONFIRM_DELETE; ?>')" href="<?php echo ADMIN_URL; ?>uploads-remote/delete/<?php echo $row['upload__ID']; ?>/" target="remote"><img border="0" src="<?php echo BASE_URL; ?>sulata/images/delete.png" title="<?php echo DELETE_RECORD; ?>"/></a>
                                                    <?php } ?>
                                            </td>
                                            <?php } ?>
...
<!-- /TABLE -->
                    <?php
                                $sqlP = "SELECT COUNT(upload__ID) AS totalRecs $sqlFrom $where";
                                suPaginate($sqlP);
                                ?>

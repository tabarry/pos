...
$sqlSelect = "SELECT product__ID,category__Category AS product__Category,product__Picture,product__Code,product__Name,product__Price,product__Status ";
$sqlFrom = " FROM sulata_products,sulata_categories WHERE product__dbState='Live' AND category__dbState='Live' AND product__Category=category__ID";
$sql = $sqlSelect . $sqlFrom;

//Download CSV
if (suSegment(1) == 'stream-csv' && $downloadAccessCSV == TRUE) {
    $outputFileName = 'products.csv';
    $headerArray = array('Category', 'Picture', 'Code', 'Name', 'Price', 'Status');
                            ...
                            <div id="content-area">
                                ...
                                <form class="form-horizontal" name="searchForm" id="searchForm" method="get" action="">
                                    <fieldset id="search-area1">
                                        <label class="col-xs-12 col-sm-12 col-md-12 col-lg-12"><i class="fa fa-search blue"></i> Search by Name or Code</label>
                                        ...
                                        <?php if ($_GET['q']) { ?>
                                            ...
                                            <div class="pull-right"><a style="text-decoration:underline !important;" href="<?php echo ADMIN_URL; ?>products.php/">Clear search.</a></div>
                                            </div>
                                        <?php } ?>
                                    </fieldset>
                                </form>
                                ...
                                <table width="100%" class="table table-hover table-bordered tbl">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:14%">Price<sup><?php echo $getSettings['site_currency']; ?></sup></th>
...
//Get records from database
                                        ...
                                        while ($row = suFetch($result)) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo suUnstrip($row['product__Category']); ?></td>
                                                <td><?php
                                                ...
                                                <td><?php echo suUnstrip($row['product__Code']); ?></td>
                                                <td><?php echo suUnstrip($row['product__Name']); ?></td>
                                                <td><?php echo number_format($row['product__Price'], 2); ?></td>
                                                <td><?php echo suUnstrip($row['product__Status']); ?></td>

                                                <?php if (($editAccess == TRUE) || ($deleteAccess == TRUE)) { ?>
                                                    <td style="text-align: center;">
                                                        <?php if ($editAccess == TRUE) { ?>
                                                            <a href="<?php echo ADMIN_URL; ?>products-update.php/<?php echo $row['product__ID']; ?>/"><img border="0" src="<?php echo BASE_URL; ?>sulata/images/edit.png" title="<?php echo EDIT_RECORD; ?>"/></a>
                                                        <?php } ?>
                                                        <?php if ($deleteAccess == TRUE) { ?>
                                                            <a onclick="return delRecord(this, '<?php echo CONFIRM_DELETE; ?>')" href="<?php echo ADMIN_URL; ?>products-remote.php/delete/<?php echo $row['product__ID']; ?>/" target="remote"><img border="0" src="<?php echo BASE_URL; ?>sulata/images/delete.png" title="<?php echo DELETE_RECORD; ?>"/></a>
                                                        <?php } ?>
                                                    </td>
                                                <?php } ?>
                                ...
                                <!-- /TABLE -->
                                <?php
                                $sqlP = "SELECT COUNT(product__ID) AS totalRecs $sqlFrom $where";
                                suPaginate($sqlP);
                                ?>

...
if (suSegment(1) == 'stream-csv' && $downloadAccessCSV == TRUE) {
    $outputFileName = 'order-details.csv';
    $headerArray = array('Order', 'Product', 'Code', 'Name', 'Price', 'Quantity');
    suSqlToCSV($sql, $headerArray, $outputFileName);
    exit;
}
//Download PDF
if (suSegment(1) == 'stream-pdf' && $downloadAccessPDF == TRUE) {
    $outputFileName = 'order-details.pdf';
                                ...
                                <form class="form-horizontal" name="searchForm" id="searchForm" method="get" action="">
                                    <fieldset id="search-area1">
                                        ...
                                        <?php if ($_GET['q']) { ?>
                                            ...
                                            <div class="pull-right"><a style="text-decoration:underline !important;" href="<?php echo ADMIN_URL; ?>order-details/">Clear search.</a></div>
                                            </div>
                                        <?php } ?>
                                    </fieldset>
                                </form>
...
//Get records from database
                                        ...
                                        while ($row = suFetch($result)) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo suUnstrip($row['orderdet__Order']); ?></td>
                                                <td><?php echo suUnstrip($row['orderdet__Product']); ?></td>
                                                <td><?php echo suUnstrip($row['orderdet__Code']); ?></td>
                                                <td><?php echo suUnstrip($row['orderdet__Name']); ?></td>
                                                <td><?php echo suUnstrip($row['orderdet__Price']); ?></td>
                                                <td><?php echo suUnstrip($row['orderdet__Quantity']); ?></td>

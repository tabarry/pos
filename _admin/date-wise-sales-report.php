...
checkLogin();
$pageName = 'Date-wise Sales Summary';
$pageTitle = 'Date-wise Sales Summary';
$downloadAccess = TRUE;
$sql = "SELECT order__ID, order__UID,order__Number,DATE_FORMAT(order__Date, '%b %d, %y %h:%i %p') AS order__Date2,CONCAT(order__Customer_Name,' - ',order__Mobile_Number) as Customer,(order__Total_Amount-order__Discount) AS Total,SUM(product__Cost_Price * orderdet__Quantity) AS costPrice,order__Tax,order__Tax_Value,order__Location FROM sulata_orders INNER JOIN sulata_order_details ON order__UID = orderdet__Order_UID INNER JOIN sulata_products ON product__ID = orderdet__Product  WHERE order__dbState='Live' AND ((order__Status = 'Received') OR (order__Status = 'Delivered')) ";
